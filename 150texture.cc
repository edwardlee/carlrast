...
void texSample(const texTexture *tex, double s, double t, double (&sample)[]) {
    ...
    if (tex->filtering == texNEAREST)
        texGetTexel(tex, round(u), round(v), sample);
    else {
	    int d = tex->texelDim;	
        // 0,0 , 0,1 , 1,0 , 1,1	
        double ff[d]; double fc[d]; double cf[d]; double cc[d];	
        texGetTexel(tex, u, v, ff);	
        texGetTexel(tex, u, ceil(v), fc);	
        texGetTexel(tex, ceil(u), v, cf);	
        texGetTexel(tex, ceil(u), ceil(v), cc);	
        // m for mantissa	
        double um = u - floor(u);	
        double vm = v - floor(v);	
